...
def ensure_default_master() -> None:
    ...
    if row:
        return
    create({"name": "Мастер", "role": "master", "characterId": None, "pin": ""})
...
def list_public() -> list[dict[str, Any]]:
    ...
    ensure_default_master()
    rows = get_db().execute(
        "SELECT id, name, role, character_id, pin_hash FROM accounts ORDER BY role DESC, name COLLATE NOCASE"
    ).fetchall()
...
def get_by_id(account_id: str) -> dict[str, Any] | None:
    row = get_db().execute(
        "SELECT id, name, role, character_id, pin_hash FROM accounts WHERE id = ?",
        (account_id,),
    ).fetchone()
...
def unlink_character(character_id: str) -> None:
    get_db().execute(
        """
        UPDATE accounts
...
def login(account_id: str, pin: str) -> tuple[dict[str, Any], str] | None:
    row = _get_row(account_id)
    if not row:
        return None
...
def session_account(token: str | None) -> dict[str, Any] | None:
    ...
    row = get_db().execute(
        """
        SELECT s.token, s.expires_at, a.id, a.name, a.role, a.character_id, a.pin_hash
        FROM sessions s
